...
export const useNotificationSettings = () => {
  ...
  const {
    ...
    error: fetchErrorCustomer,
  } = useGetCustomer();
  const {
    data: proExpertData,
    isLoading: isLoadingExpert,
    error: fetchErrorExpert,
  } = useGetProExpert();
  ...
  const updateMutation = isClient
    ? updateCustomerMutation
    : updateProExpertMutation;
    ...
    if (userData) {
      setSettings({
        appointment_notification_sms:
          userData.appointment_notification_sms ?? true,
        appointment_notification_email:
          userData.appointment_notification_email ?? true,
        message_notification_sms: userData.message_notification_sms ?? true,
        message_notification_email:
          userData.message_notification_email ?? false,
        promotions_notification_sms:
          userData.promotions_notification_sms ?? false,
        promotions_notification_email:
          userData.promotions_notification_email ?? true,
      });
    }
  }, [userData]);
  ...
  const getSmsNotifications = useCallback(() => {
    return NOTIFICATION_CONFIG.filter(
      (config: NotificationItem) => config.type === "sms"
  ...
  const getEmailNotifications = useCallback(() => {
    return NOTIFICATION_CONFIG.filter(
      (config: NotificationItem) => config.type === "email"
    ).map((config: NotificationItem) => ({
      id: config.key,
      icon: config.icon,
      label: config.label,
  ...
  const handleSmsNotificationChange = useCallback(
